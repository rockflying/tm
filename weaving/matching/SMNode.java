...
public class SMNode implements State {
 ...
 //--------------------------------------------------------------------------------------------------------------   
     ...
     * Determines whether this node has an outgoing (non-skiploop) edge with a given label.
     * @param label the label to consider
     * @return <code>true</code> if an edge with the label exists <code>false</code> otherwise.
     */
    public boolean hasOutEdgeWithLabel(String label) {
    	SMEdge edge;
     ...
     * Should perhaps be changed to return a set of edges, in the case of non-deterministic systems
     * Currently this method is used for generating states when merging StateMachines.
     * @param label
     * @return the outgoing edge with with label "label", if it exists, otherwise, <code>null</code>
     */
    public SMEdge getOutEdgeWithLabel(String label) {
    	SMEdge edge;
    	Iterator it = outgoing.iterator();
    	while(it.hasNext()) {
    		edge = (SMEdge) it.next();
    ...
    /**
     * @author Caitlin Phillips
     * Finds the state to which the system transitions under a given label. 
     * Should perhaps be changed to return a set of edges, in the case of non-deterministic systems
     * Currently this method is used for generating states when merging StateMachines.
     * @param label
     * @return the target of the edge with with label "label", if it exists, otherwise, <code>null</code>
     */
     
    public SMNode getTransitionUnderLabel(String label) {
    	SMEdge edge;
    	Iterator it = outgoing.iterator();
    	while(it.hasNext()) {
    		edge = (SMEdge) it.next();
    ...
    /**
     * @author Caitlin Phillips
     * Determines whether there exists an edge to a node with a given label and number
     * @param to The target node
     * @param label the label to check for
     * @param TMnum the number of the tracematch
     * @return <code>true</code> if such an edge exists, <code>false</code> otherwise. 
     */
    public boolean hasNumberedEdgeTo(SMNode to, String label, int TMnum) {
        SMEdge edge;
        SMNumberedEdge nEdge;
        Iterator it = outgoing.iterator();
        while(it.hasNext()) {
            edge = (SMEdge)it.next();
            if(edge.isNumbered()&& !edge.isSkipEdge())
            {
            	nEdge = (SMNumberedEdge) edge;
            	if(nEdge.getTarget() == to && !nEdge.isSkipEdge() && nEdge.belongsTo(TMnum) &&
            		(nEdge.getLabel() == label || (label != null && label.equals(edge.getLabel())))) 
            		return true;
            }
        }
        return false;
