...
public class TMStateMachine implements StateMachine {
 ...
 //------------------------------------------------------------------------------------------------------------------ 
    /**
     * Adds a new numbered edge numbered with n
     * Assumes the from and to variables are actually the relevant implementations from
     * the abc.tm.weaving.matching package -- will throw ClassCastException otherwise
     * @author Caitlin Phillips
     */
    public SMEdge newNumberedTransition(State from, State to, String s, int n) {
        SMNode f = (SMNode)from;
        SMNode t = (SMNode)to;
        SMEdge edge = new SMNumberedEdge(f, t, s, n);
        f.addOutgoingEdge(edge);
        t.addIncomingEdge(edge);
        edges.add(edge);
        return edge;
    ...
    /** 
     * Adds a new numbered edge numbered with TMnums (a set)
     * Assumes the from and to variables are actually the relevant implementations from
     * the abc.tm.weaving.matching package -- will throw ClassCastException otherwise
     * @author Caitlin Phillips
     */
    public SMEdge newNumberedTransition(State from, State to, String s, Set<Integer> TMnums) {
        SMNode f = (SMNode)from;
        SMNode t = (SMNode)to;
        SMEdge edge = new SMNumberedEdge(f, t, s, TMnums);
        f.addOutgoingEdge(edge);
        t.addIncomingEdge(edge);
        edges.add(edge);
        return edge;
    ...
    /**
     * Adds a new numbered skiploop numbered with n
     * Assumes the from and to variables are actually the relevant implementations from
     * the abc.tm.weaving.matching package -- will throw ClassCastException otherwise
     * @author Caitlin Phillips
     */
    public SMEdge newNumberedSkipLoop(State state, String label, int n ) {
        SMNode s = (SMNode)state;
        SMEdge edge = new NumberedSkipLoop(s, label, n);
    ...
    /** 
     * Adds a new numbered skiploop numbered with TMnums (a set)
     * Assumes the from and to variables are actually the relevant implementations from
     * the abc.tm.weaving.matching package -- will throw ClassCastException otherwise
     * @author Caitlin Phillips
     */
    public SMEdge newNumberedSkipLoop(State state, String label, Set<Integer> TMnums) {
        SMNode s = (SMNode)state;
        SMEdge edge = new NumberedSkipLoop(s, label, TMnums);
        s.addOutgoingEdge(edge);
        s.addIncomingEdge(edge);
        edges.add(edge);
        return edge;
...
//-----------------------------------------------------------------------------------------------------------------------    
 ...
 //--------------------------------------------------------------------------------------------------------   
    /**
     * Part of the automaton construction -- A numbered skip loop s_i added on every state q if there is no "normal" 
     * s_i loop on q in the merged automaton (where s is a symbol name and i an
     * automaton number) *and* s is a declared symbol of tracematch i.
     * 
     * @param symMap: a HashMap between tracematch numbers and their symbolsets.
     * This assumes that no state already has a skip loop. Skips are empty labels (as opposed
     * to null labels, which represent epsilon transitions -- those should have been eliminated).
     * @author Caitlin Phillips
     */
    protected void addNumberedSelfLoops(HashMap<Integer, Set<String>> symMap) {
    	//get all symbols
    	Set<String> declaredSymbols = new HashSet<String>();
    	for(Integer key : symMap.keySet())
    	{
    		declaredSymbols.addAll(symMap.get(key));
    	}
        ...
        while(it.hasNext()) {
	        ...
	        if(!cur.isInitialNode()) { 
	        	...
	        	for (Iterator symIter = declaredSymbols.iterator(); symIter.hasNext();) {
	        		String symbolName = (String) symIter.next();
	        		HashSet<Integer> TMnums = new HashSet<Integer>();
	        		//for each tracematch
	        		for(Iterator tmIter = symMap.keySet().iterator(); tmIter.hasNext();){
	        			
	        			Integer tm = (Integer) tmIter.next();
	        			//... in which there does not exist a loop already at the state...
	        			//...and symbolName is declared in tm
	        			if(!cur.hasNumberedEdgeTo(cur, symbolName, tm.intValue()) && symMap.get(tm).contains(symbolName) ) {
						//... add tm to the list of tracematches for which a skip loop is needed
	        				TMnums.add(tm);
	        			}
	        		}
	        		
	        		//add a skip loop for symbolName, for all tracematches for which it is needed
	        		if(!TMnums.isEmpty())
	        			newNumberedSkipLoop(cur, symbolName, TMnums);
	        	}
	        }
        }
    }
//--------------------------------------------------------------------------------------------------------   
     ...
     * 
     * Also:
     * When merging stateMachines, the traceMatch numbers from the original systems,
     * are copied to the appropriate edges in the determinised system.
     * -Caitlin Phillips (03/26/2008)
     */
    protected TMStateMachine determinise() {
    		...
    		result.renumberStates();
    		//System.err.println("Determinized TM:\n"+result.toString());
    		//collect SYmbols in determinized SM
    		HashSet<String> symbolSet = new HashSet<String>();
    		for(Iterator nodeIt = result.getStateIterator(); nodeIt.hasNext();){
    			SMNode aNode = (SMNode) nodeIt.next();
    			for( Iterator edgeIt = aNode.getOutEdgeIterator(); edgeIt.hasNext();){
    				SMEdge anEdge = (SMEdge) edgeIt.next();
    				if(!symbolSet.contains(anEdge.getLabel()))
    					symbolSet.add(anEdge.getLabel());
    			}
    		}
    		//Update edge numbers:
    		//for each node in the new SM
    		for(Iterator setIt = nodeMap.keySet().iterator(); setIt.hasNext();){
    		
    			//get the corresponding list of old nodes
    			LinkedHashSet oldNodes = (LinkedHashSet)setIt.next();
    			//System.err.println(oldNodes.toString());
    			SMNode newState = (SMNode) nodeMap.get(oldNodes);
    			//for each symbol
    			for(Iterator symIt = symbolSet.iterator(); symIt.hasNext();){
    				String sym = (String) symIt.next();
    				if(! newState.hasOutEdgeWithLabel(sym))
    					continue;
    				HashSet<Integer> TMnums = new HashSet<Integer>();
    				HashSet<SMEdge> toRemove = new HashSet<SMEdge>();
    				//for each node, collect all numbers on edges labelled with sym
    				for(Iterator nodeIt = oldNodes.iterator(); nodeIt.hasNext();){
    					SMNode aNode = (SMNode) nodeIt.next();
    					for(Iterator edgeIt = aNode.getOutEdgeIterator(); edgeIt.hasNext();){
    						SMEdge e = (SMEdge) edgeIt.next();
    						if( e.getLabel().equals(sym) && e.isNumbered()){
        						SMNumberedEdge eNum = (SMNumberedEdge) e;
        						TMnums.addAll(eNum.getTMList());
        						//toRemove.add(eNum);
    				...
    				//now add a new numbered edge:
    				//Note: if none of the edges of the original system are numbered,
    				//then TMnums will be empty, and thus, no numbered edges will be created 
    				//in the new system.
    				if(!TMnums.isEmpty())
    				{
    				SMNode to = newState.getTransitionUnderLabel(sym);
    				toRemove.add(newState.getOutEdgeWithLabel(sym));
    				result.newNumberedTransition(newState, to, sym, TMnums);
    				result.removeEdges(toRemove);
    				}
    				//System.err.println(nodeMap.);
    				//System.err.println(sym+": "+TMnums.toString());
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------
	...
	/**
	 * Helper method for discovering states in the merged system
	 * @author Caitlin Phillips
	 */
	private void genStates(State oldInit, State mergedInit, Set<String> SymbolSet, int tmNum){
		SMNode oState, mState, oDest, mDest;
		
		Stack<State> oStack = new Stack<State>();
		Stack<State> mStack = new Stack<State>();
		//This could probably be done more elegantly with a hashMap
		oStack.push(oldInit);
		mStack.push(mergedInit);
		//while there are elements on the stack
		while(!oStack.isEmpty() && !mStack.isEmpty()){
			//pop the top elements
			oState = (SMNode) oStack.pop();
			mState = (SMNode) mStack.pop();
			//for every symbol
			for(String sym : SymbolSet){
				//find all transitions in the old state and create them in the new one
				if(oState.hasOutEdgeWithLabel(sym)){
					oDest = oState.getTransitionUnderLabel(sym);
					if(!(oDest==oState))
						mDest = (SMNode) this.newState();
					else
						mDest = mState;
					this.newNumberedTransition(mState, mDest, sym, tmNum);
					
					//if the destination is not a final state and the transition is not a self-loop,
					//push the state we just created, and its corresponding state in the original system onto the stack.
					if(!oDest.isFinalNode()&& !(oDest==oState)){
						oStack.push(oDest);
						mStack.push(mDest);
					}
					else if(oDest.isFinalNode())
						mDest.setFinal(true);
	...
	/**
	 * Merges two traceMatches
	 * @param TM2: the second traceMatch to be merged
	 * @param SymbolSet1: The symbols from TM1
	 * @param SymbolSet2: the symbols from TM2
	 * @return mergedTM: the merged machine
	 * @author Caitlin Phillips
	 */
	public TMStateMachine merge(TMStateMachine TM2, Set<String> SymbolSet1, Set<String> SymbolSet2){
		Set<String> SymbolSet = new HashSet<String>();
		SymbolSet.addAll(SymbolSet1);
		SymbolSet.addAll(SymbolSet2);
		
		TMStateMachine TM1 = this;
		TMStateMachine mergedTM = new TMStateMachine();
		
		SMNode mergedInit = (SMNode) mergedTM.newState();
		mergedInit.setInitial(true);
		
		Set<SMNode> TM1InitStates = TM1.getInitialStates();
		Set<SMNode> TM2InitStates = TM2.getInitialStates();
		
		for(SMNode init1 : TM1InitStates){
			mergedTM.genStates(init1, mergedInit, SymbolSet, 1);
		}
		
		for(SMNode init2 : TM2InitStates){
			mergedTM.genStates(init2, mergedInit, SymbolSet, 2);
		}
		mergedTM.renumberStates();
		//System.err.println("Undeterminized TM:\n"+mergedTM.toString());
		Set<Integer> TMSet = new HashSet<Integer>();
		TMSet.add(new Integer(1));
		TMSet.add(new Integer(2));
		//mergedTM.addNumberedSelfLoops(SymbolSet, TMSet);
		mergedTM.minimize();
		HashMap<Integer, Set<String>> symMap = new HashMap<Integer, Set<String>>();
		symMap.put(new Integer(1), SymbolSet1);
		symMap.put(new Integer(2), SymbolSet2);
		mergedTM.addNumberedSelfLoops(symMap);
		mergedTM.removeSkipToFinal();
		//mergedTM.compressStates();
		mergedTM.renumberStates();
		mergedTM.splitFinalStates(SymbolSet, TMSet);
		//mergedTM.toString();
		//Clean up mergedTM
	...
	/**
	 * splits final states after merging tracematches
	 * @param symbols : the symbols being used
	 * @param TMs: the tracematch numbers
	 * @author Caitlin Phillips
	 */
	public void splitFinalStates(Set<String> symbols, Set<Integer> TMs){
		SMEdge e;
		SMNumberedEdge eNum;
		SMNode to, from;
		Integer key;
		HashSet<SMNode> finalStates = new HashSet<SMNode>();
		for(Iterator stateIt = this.getStateIterator(); stateIt.hasNext(); )
		{
			SMNode aState = (SMNode) stateIt.next();
			if(aState.isFinalNode())
				finalStates.add(aState);
		}
		
		boolean noSharedFinalStates = true;
		
		for(SMNode fState : finalStates ){
			for(Iterator edgeIt = fState.getInEdgeIterator(); edgeIt.hasNext();){
				e = (SMEdge) edgeIt.next();
				if(e.isNumbered() && !e.isSkipEdge()){
					eNum = (SMNumberedEdge) e;
					noSharedFinalStates = noSharedFinalStates && (eNum.getTMList().size()==1);
				}else{
					noSharedFinalStates= false;
				}
			}
		}
		
		//if noSharedFinal states:
		//Create hashmap TMnum --> newFinal state
		//add edges
		//remove old final states
		if(noSharedFinalStates){
			HashSet<SMEdge> toRemove = new HashSet<SMEdge>();
			HashMap<Integer, SMNode> tmStateMap = new HashMap<Integer, SMNode>();
			for(Integer i : TMs)
				tmStateMap.put(i, (SMNode) this.newState());
			System.err.println(tmStateMap.keySet().toString());
			for(SMNode fin : finalStates){
				for(String sym: symbols){
					for(Iterator inEdgeIt = fin.getInEdgeIterator(); inEdgeIt.hasNext();){
						eNum = (SMNumberedEdge) inEdgeIt.next();
						if(eNum.label.equals(sym)){
							from = eNum.getSource();
							key = eNum.getTMList().iterator().next();
							to = tmStateMap.get(key);
							this.newNumberedTransition(from, to, sym, TMs); //could possibly add non-numbered edge here
							toRemove.add(eNum);
						}
					}
				}
			}
			nodes.removeAll(finalStates);
			this.removeEdges(toRemove);
			for(Integer i: tmStateMap.keySet()){
				tmStateMap.get(i).setFinal(true);
			}
			this.renumberStates();
		}
	}
